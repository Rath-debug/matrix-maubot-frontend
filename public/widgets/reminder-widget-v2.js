...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const calendarTimeBoost = document.getElementById('calendarTimeBoost');
    const calendarTimeNote = document.getElementById('calendarTimeNote');
    const calendarTimeChips = Array.from(document.querySelectorAll('.calendar-time-chip'));
    ...
    function computePresetTime(preset) {
        if (preset !== 'now+15') {
            return preset;
        }
        ...
        now.setMinutes(rounded + 15, 0, 0);
        return formatClockFromMinutes((now.getHours() * 60) + now.getMinutes());
    }

    function updateTimeMarketingCopy(timeValue) {
        if (!calendarTimeBoost || !calendarTimeNote) return;
        if (!timeValue) {
            calendarTimeBoost.textContent = 'Prime Window';
            calendarTimeNote.textContent = 'Choose a launch slot to maximize visibility.';
            return;
        }

        const [h, m] = timeValue.split(':').map((v) => parseInt(v, 10) || 0);
        const minuteOfDay = (h * 60) + m;

        if (minuteOfDay < 720) {
            calendarTimeBoost.textContent = 'Morning Push';
            calendarTimeNote.textContent = 'Early slots help your reminder land before busy hours.';
        } else if (minuteOfDay < 1020) {
            calendarTimeBoost.textContent = 'Peak Attention';
            calendarTimeNote.textContent = 'Midday timing usually gets the fastest response.';
        } else {
            calendarTimeBoost.textContent = 'Evening Follow-up';
            calendarTimeNote.textContent = 'Late-day slots are ideal for wrap-up and follow-through.';
        }
    }

    function setActiveTimeChip(timeValue) {
        calendarTimeChips.forEach((chip) => {
            const preset = chip.getAttribute('data-time-preset');
            const presetValue = computePresetTime(preset);
            const isActive = preset !== 'now+15' && presetValue === timeValue;
            chip.classList.toggle('active', isActive);
        });
    }

    if (calendarTimeInput) {
        calendarTimeInput.addEventListener('input', function() {
            updateTimeMarketingCopy(calendarTimeInput.value);
            setActiveTimeChip(calendarTimeInput.value);
        });
    ...
    calendarTimeChips.forEach((chip) => {
        chip.addEventListener('click', function() {
            if (!calendarTimeInput) return;
            const preset = chip.getAttribute('data-time-preset');
            const timeValue = computePresetTime(preset);
            calendarTimeInput.value = timeValue;
            updateTimeMarketingCopy(timeValue);
            setActiveTimeChip(timeValue);
            if (preset === 'now+15') {
